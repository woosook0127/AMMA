...
def evaluate(data_loader, model, device, dist_eval=False):
    ...
    if outputs and targets:
        ...
        print("No valid outputs or targets for evaluation.")
        return {"mAP": 0.0, "AP": []}

    # Save inference results (optional)
    np.save('inf_output.npy', {'vids': vids, 'embs_527': outputs, 'targets': targets})

    # Calculate statistics
    stats = calculate_stats(outputs, targets)

    # Filter out invalid AP values
    valid_AP = [stat['AP'] for stat in stats if not np.isnan(stat['AP'])]
    if valid_AP:
        mAP = np.mean(valid_AP)
    else:
        print("No valid AP values for mAP calculation.")
        mAP = 0.0

    print(f"mAP: {mAP:.6f}")
    return {"mAP": mAP, "AP": valid_AP}
