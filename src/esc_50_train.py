...
if __name__ == "__main__":
    log_dir = "../output/logs/esc_50"
    writer = SummaryWriter(log_dir)
    esc_path = "/home/kimdoyoung/AudioMAE/my_dataset/ESC50/"
    df = pd.read_csv(esc_path + 'meta/esc50.csv')
    
    train = df[df['fold']!=5]
    valid = df[df['fold']==5]
    
    train_data = ESC50Data(esc_path+'audio', train, 'filename', 'category')
    valid_data = ESC50Data(esc_path+'audio', valid, 'filename', 'category')
...
if __name__ == "__main__":
    ...
    training(model, loss_fn, train_loader, valid_loader, epochs, optimizer, train_losses, valid_losses, lr_decay)
