...
class Patches(nn.Module):
    ...
    def forward(self, mfccs):
        ...
        # print(f"mfcc_s in Patches forwarding : {mfccs.shape}")
        patches = mfccs.unfold(2, self.patch_size, self.patch_size).unfold(3, self.patch_size, self.patch_size)
        patches = patches.contiguous().view(batch_size, -1, self.patch_size * self.patch_size * mfccs.size(1))
...
class AMMA(nn.Module):
    def __init__(self, patch_size, num_patches, hidden_dim, token_mixing_dim, channel_mixing_dim, num_of_mlp_blocks, n_mfcc, time_frames, num_classes):
        super(AMMA, self).__init__()
        self.patch_size = patch_size
        ...
        # MFCC-specific Data Augmentation
        self.data_augmentation = nn.Sequential(
            T.Vol(0.1),  # Random volume adjustment
            nn.Dropout(0.1),
        )
    ...
    def forward(self, mfccs):
        ...
        # mfccs Padding
        h_pad = (self.patch_size - mfccs.shape[1] % self.patch_size) % self.patch_size
        w_pad = (self.patch_size - mfccs.shape[2] % self.patch_size) % self.patch_size
        mfccs = F.pad(mfccs, (0, w_pad, 0, h_pad), mode='constant', value=0)
        
        augmented_mfccs = self.data_augmentation(mfccs) # Data augmentation
        # print(f"augmented_spec = {augmented_mfccs.shape}")
        X = self.patches(augmented_mfccs) # Extract patches from spectrograms
...
if __name__ == "__main__":
    patch_size = 10
    n_mfcc = 40         # number of frequency bins in the spectrogram
    time_frames = 640       # number of time frames in the spectrogram
    num_patches = (n_mfcc // patch_size) * (time_frames // patch_size)  # number of patches
    ...
    model = AMMA(patch_size, num_patches, hidden_dim, token_mixing_dim, channel_mixing_dim, num_of_mlp_blocks, n_mfcc, time_frames, num_classes)

    # Test with a random batch of spectrograms
    spectrograms = torch.rand(32, 1, n_mfcc, time_frames)  # (batch_size, 1 channel, n_mfcc, time_frames)
