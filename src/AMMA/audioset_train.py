...
from AMMA import AMMA  # MLP-Mixer for Audio model
...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        st = time.perf_counter()
        mel_tensor = self.transform(y).unsqueeze(0)
...
def validate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(dataloader, desc="Validation Batch", leave=False):
            ...
            # print(f"###### Outputs: {outputs.shape}: {outputs.mean()}, {outputs.min()}, {outputs.max()}")
            # print(f"OOOOOOOOOOOOO Label: {labels.shape}:{labels.mean()}, {labels.min()}, {labels.max()}")
            # print(f"#### True Label이 없습니다 ####") if labels.any() != 1 else None
            # Append outputs and labels for metric calculation
...
if __name__ == "__main__":
    ...
    n_mfcc              = 40
    time_frames         = 621
    num_patches         = (n_mfcc // patch_size) * (time_frames // patch_size)
    hidden_dim          = 768
        ...
        for epoch in range(epochs):
            ...
            for inputs, labels in train_loader:
                ...
                if step % 500 == 0:  # 500 step마다 저장
                    ...
                    torch.save(model.state_dict(), f"{output_dir}/AMMA_AS_{step:05d}.pth")

            # Validation at the end of each epoch
            eval_loss, metrics = validate(model, eval_loader, criterion, device)
            writer.add_scalar("Loss/Eval", eval_loss, step)
            writer.add_scalar("Loss/Eval", eval_loss, step)
            writer.add_scalar("mAP/Eval", metrics["mAP"], epoch)
            writer.add_scalar("Micro F1/Eval", metrics["Micro F1"], epoch)
            writer.add_scalar("Macro F1/Eval", metrics["Macro F1"], epoch)
